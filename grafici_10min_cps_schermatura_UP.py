...
def plotInitializer(ax):
	...
	ax.set_title("SCHERMATURA SOPRA")
...
for direzione in range(2):
	...
	for i in particlesIndexer:
		...
		if direzione == 0:
			...
			print("Il valore medio NON SCHERMATO è ", valoreMedio, "\n")
			ax.text(0.50, 0.9, r'$\bar x$ NON SCHERMATO = ' + str(valoreMedio)[:8], color=(.1,.1,1,1), horizontalalignment='center', verticalalignment='center', transform=ax.transAxes)
		elif direzione == 1:
			valoreMedio = avg(particles10min)
			print("Il valore medio SCHERMATO è ", valoreMedio, "\n")
			ax.text(0.50, 0.86, r'$\bar x$ SCHERMATO = ' + str(valoreMedio)[:8], color=(1,.1,.1,1), horizontalalignment='center', verticalalignment='center', transform=ax.transAxes)
